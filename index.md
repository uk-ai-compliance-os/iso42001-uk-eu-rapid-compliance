# ISO 42001 UK-EU Rapid Compliance Toolkit

**Open-source AI Management System (AIMS) architecture for UK AI companies facing the August 2, 2026 EU AI Act enforcement deadline.**

[View on GitHub](https://github.com/uk-ai-compliance-os/iso42001-uk-eu-rapid-compliance) | [Download Toolkit](https://github.com/uk-ai-compliance-os/iso42001-uk-eu-rapid-compliance/archive/refs/heads/main.zip) | [Chat-Only Consulting](https://github.com/uk-ai-compliance-os/iso42001-uk-eu-rapid-compliance/blob/main/CONSULTING.md)

---

## What You Get (Free, MIT Licensed)

| Component | UK AI Companies |
|-----------|---------------|
| **5 Sector Templates** | Fintech, Healthtech, SaaS, Legaltech, Insurtech AI policies |
| **Gap Analysis CLI** | Python tool: 10 questions → ISO 42001 + EU AI Act gap report |
| **MLflow Governance Hook** | Auto-log model training for audit evidence |
| **LangChain Observability** | LLM transparency logging for EU AI Act Article 52 |
| **Risk Registers** | Pre-built CSV with ISO 42001 control mapping |
| **EU AI Act Crosswalk** | ISO 42001 clauses mapped to EU AI Act articles |
| **UK Regulatory Guide** | ICO, FCA, MHRA alignment documentation |
| **August 2026 Sprint Guide** | 12-week week-by-week compliance plan |

---

## Who This Is For

- UK fintech using AI for credit scoring or fraud detection
- UK healthtech with clinical decision support or diagnostic AI
- UK SaaS platforms serving EU enterprise customers
- UK legaltech deploying contract analysis or litigation prediction
- UK insurtech automating claims or underwriting

**CTOs, Compliance Leads, and Founders** who need certification-ready documentation without a £100k Big Four engagement.

---

## The August 2026 Problem (76 Days Remaining)

The EU AI Act applies extraterritorially to UK AI providers and deployers with EU exposure. High-risk system enforcement begins **August 2, 2026**. Penalties: up to **€35 million or 7% of global turnover**.

Most UK AI companies currently have:
- No documented AI policy
- No EU AI Act risk classification
- No ISO 42001-aligned governance structure
- No technical documentation for conformity assessment

This toolkit closes those gaps in days, not months.

---

## Quick Start

```bash
git clone https://github.com/uk-ai-compliance-os/iso42001-uk-eu-rapid-compliance.git
cd iso42001-uk-eu-rapid-compliance/scripts
python gap-analysis-cli.py
Answer 10 prompts. Receive a red/amber/green readiness report in 2 minutes.
| Service                                | Investment | Timeline |
| -------------------------------------- | ---------- | -------- |
| EU AI Act Scope Diagnostic             | £2,500     | 48 hours |
| ISO 42001 Rapid Roadmap                | £8,000     | 10 days  |
| Full AIMS Build + Certification Sprint | £15,000    | 30 days  |
| August 2026 Emergency Sprint           | £20,000    | 14 days  |
No calls. No Calendly. No meetings.
📧 compliance.architect@protonmail.com
⏱️ Response: < 4 hours
Built for UK AI companies racing the August 2, 2026 deadline. MIT License.
